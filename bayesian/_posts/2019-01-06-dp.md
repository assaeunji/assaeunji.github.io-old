...
* 유한 집합 $$\mathbf{A}=(A_1,\ldots,A_r)$$는 exclusive & exhaustive한 성질을 가진 partition이다. exclusive하고 exhasutive하다는 말은 아래의 사진처럼, $$A_1,\ldots, A_6$$은 겹치지 않고 ($$A_1\cap A_2\cap \cdots \cap A_r$$), 전체의 합집합이 전체집합 ($$A_1\cup A_2 \cup \cdots \cup A_r = \mathbf{A}$$))이다.
