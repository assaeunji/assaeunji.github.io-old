---
layout: post
title: "[Dirichlet process] 디리슐레 과정 (1)"
date: 2019-01-06 
categories: [Bayesian]
tag: [dirichlet, bayesian, multinomial, DP]
---

### **[Dirichlet process] 디리슐레 과정 (1)**
...
* 이후, `cumprod` 메쏘드를 이용해 누적곱을 계산해준다. 이때 `axis=1`은 행의 축을 기준으로 곱해준다는 의미로, $$K-1$$개의 $$\beta$$를 곱해주는 역할을 한다.

$$
\begin{aligned}
\pi_1 &= \beta_1\\
\pi_r &= \beta_r \cdot \prod^{K-1}_{i=1}(1-\beta_i),\ r=2,\cdots,K
\end{aligned}
$$
...
* `sample_cdfs`: 마지막으로 이를 `axis=1` (행의 축)으로 더해줌으로써 
$$G = \sum_{r=1}^K\pi_r \delta_{\theta_r}$$를 계산해준다.
