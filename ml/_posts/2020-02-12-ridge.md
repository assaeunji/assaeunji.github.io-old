...
* $$p$$개의 설명변수가 있으면 이의 모수 공간은 $$\mathbb{R}^p$$라 일컫는데, Ridge regression은 이를 
$$
...
![](../images/ridge-graph.png)
  * 이 그림에서 빨간색 타원은 $$\boldsymbol{\beta} = (\beta_1, \beta_2)$$의 값에 따른 RSS (Residual Sum of Squares)인 $$(\mathbf{y-X}\boldsymbol{\beta})^\top (\mathbf{y-X}\boldsymbol{\beta})$$를 의미합니다. 
...
![](../images/lasso-graph.png)
  * 이 그림에서 빨간색 타원은 $$\boldsymbol{\beta} = (\beta_1, \beta_2)$$의 값에 따른 RSS (Residual Sum of Squares)인 $$(\mathbf{y-X}\boldsymbol{\beta})^\top (\mathbf{y-X}\boldsymbol{\beta})$$를 의미합니다. 
  * 즉, $$\boldsymbol{\beta} = \widehat{\boldsymbol{\beta}}$$일 때 RSS가 가장 작은 "Least Squares" estimator입니다. 
  * 파란색 사각형은 Lasso의 제약조건인 $$\|\boldsymbol{\beta}\|_1 \leq t$$를 $$|\beta_1|+|\beta_2| \leq t$$로 표현한 것입니다. 
  * 따라서 RSS를 최소화하는 데 제약조건을 만족해야하므로, Ridge regression의 해는 빨간색 타원과 파란색 원의 접점이 됩니다. 
  * 여기서 주목할 점은 접점이 $$\beta_1$$가 $0$인 지점 (파란색 원)이라는 것입니다. 즉, Lasso regression은 Ridge와 달리 몇 개의 모수는 $$0$$으로 추정하여 sparse solution을 내는 특징을 가집니다. sparse하다는 말은 해가 "드문드문"있고 나머지는 0인 경우를 의미합니다.
