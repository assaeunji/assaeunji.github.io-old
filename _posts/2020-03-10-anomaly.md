...
정상치와 이상치 대한 라벨이 **모두** 있는 데이터를 훈련시키는 방법입니다. 그러나 이 방법은 
...
이 학습을 통해 정상치들의 경계 (boundary)를 만들어서 그 경계에 속하지 않으면 이상치로 탐지하는 방법입니다. 
...
**단점**
* 은닉층의 계층적 구조가 이상치 데이터를 대표하지 못할 수도 있기 때문에 과적합 문제가 있을 수 있음

*해석이 확실치 않아 본문을 가져왔습니다*
...
훈련을 특성을 추출하는 데 목적을 두면 Deep Hybrid Models, 
